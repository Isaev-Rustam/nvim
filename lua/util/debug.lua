-- Разрешаем использование глобальных переменных
-- selene: allow(global_usage)

local M = {}

-- Получение местоположения вызова функции
function M.get_loc()
...
function M._dump(value, opts)
    ...
    vim.notify(msg, vim.log.levels.INFO, {
        title = "Отладка: " .. opts.loc,
        on_open = function(win)
            vim.wo[win].conceallevel = 3 -- Уровень скрытия текста
            vim.wo[win].concealcursor = "" -- Скрытие курсора
            vim.wo[win].spell = false -- Отключение проверки орфографии
            local buf = vim.api.nvim_win_get_buf(win)
            if not pcall(vim.treesitter.start, buf, "lua") then
                vim.bo[buf].filetype = "lua" -- Устанавливаем тип файла Lua
            end
        end,
    })
end
...
function estimateSize(value, visited)
    ...
    -- Инициализация таблицы посещенных объектов
    ---@type table<any, true>
    visited = visited or {}

    -- Проверка на уже посещенные объекты, чтобы избежать рекурсии
    ...
    elseif type(value) == "function" then
        bytes = 32 -- Базовый размер функции
        -- Добавляем размер upvalues
        local i = 1
        while true do
    ...
    elseif type(value) == "table" then
        bytes = 40 -- Базовый размер записи таблицы
